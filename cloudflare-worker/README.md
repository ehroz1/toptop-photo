# Picta — прокси на Cloudflare Worker
...
5. Вернитесь на страницу воркера → вкладка **Settings** → **Variables and Secrets**.
   ...
   - `ALLOWED_ORIGINS` — адреса вашего сайта через запятую, без слэша на
     конце: `https://picta.cc,https://www.picta.cc,https://ehroz1.github.io`.
     Эти три адреса воркер и так пускает всегда (они прописаны в его коде),
     переменная только ДОБАВЛЯЕТ к ним новые — например, если сайт начнёт
     открываться ещё по какому-то адресу (впишите точно тот, что в адресной
     строке браузера). Проверить, пускает ли воркер сайт: GitHub → Actions →
     «Проверка сайта и воркера» → Run workflow.

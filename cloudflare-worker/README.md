...
5. Вернитесь на страницу воркера → вкладка **Settings** → **Variables and Secrets**.
   ...
   - `ALLOWED_ORIGINS` — адреса вашего сайта через запятую, без слэша на
     ...
     Если сайт открывается по другому адресу — впишите точно тот, что в
     адресной строке браузера. Если переменную не задавать, воркер берёт
     этот же список из своего кода.
